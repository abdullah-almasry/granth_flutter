...
class _LibraryScreenState extends State<LibraryScreen> with AfterLayoutMixin<LibraryScreen> {
  ...
  DownloadedBook isExists(List<DownloadedBook> tasks, BookDetail mBookDetail) {
    ...
    tasks.forEach((task) {
      if (task.bookId == mBookDetail.bookId.toString() && task.fileType == "purchased") {
        exist = task;
      }
    });
    if (exist == null) {
          ...
          DefaultTabController(
            ...
            child: Scaffold(
              appBar: getBoolAsync(IS_LOGGED_IN)
                  ...
                  : AppBar(backgroundColor: context.scaffoldBackgroundColor, iconTheme: context.theme.iconTheme, centerTitle: true, title: Text(keyString(context, "lbl_samples"), style: boldTextStyle(color: context.theme.textTheme.headline6.color))),
  ...
  onBookClick(context, DownloadedBook mSampleDownloadTask) async {
    if (await Permissions.storageGranted()) {
      if (mSampleDownloadTask.status == DownloadTaskStatus.undefined) {
        ...
        readFile(context, mSampleDownloadTask.mDownloadTask.filename, mSampleDownloadTask.bookName);
      } else {
                  ...
                  child: Stack(
                    ...
                    children: <Widget>[
                      ...
                      bookDetail.status == DownloadTaskStatus.undefined
                          ? Container(margin: EdgeInsets.all(4.0), decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.black.withOpacity(0.5)), padding: EdgeInsets.all(4.0), child: Icon(Icons.file_download, size: 14, color: Colors.white))
