...
class _WishlistScreenState extends State<WishlistScreen> with AfterLayoutMixin<WishlistScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        actions: <Widget>[
          cartIcon(context, getIntAsync(CART_COUNT)).visible(getBoolAsync(IS_LOGGED_IN)),
        ],
        title: Column(
          children: <Widget>[
            Text(keyString(context, "lbl_wish_list"), style: boldTextStyle(color: context.theme.textTheme.headline6.color)).paddingTop(12.0),
            2.height,
            Text(list.length.toString() + ' ' + keyString(context, "lbl_books"), style: primaryTextStyle(color: textColorSecondary, size: 14)),
          ],
        ),
      ),
      body: Container(
                    ...
                    return InkWell(
                      onTap: () {
                        if (getIntAsync(DETAIL_PAGE_VARIANT, defaultValue: 0) == 1) {
                          BookDescriptionScreen(bookDetail: BookDetail(bookId: data.book_id)).launch(context);
                        } else {
                          BookDescriptionScreen2(bookDetail: BookDetail(bookId: data.book_id)).launch(context);
                        }
                      },
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            ...
                            10.height,
                            Text(
                              data.name.toString().validate().capitalizeFirstLetter(),
                            ...
                            4.height,
                            Text(
                              data.author_name.toString().validate().capitalizeFirstLetter(),
