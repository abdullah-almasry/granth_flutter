...
class _CartScreenState extends State<CartScreen> with AfterLayoutMixin<CartScreen> {
  List<CartItem> list = [];
  ...
  processPayTmPayment(context) async {
    ...
    await CartPayment.payWithPayTm(context, total, getOrderDetail().toJson(), paymentMethod);
  }

  processBrainTreePayment(context, token) async {
    showLoading(true);
    CartPayment.paywithPayPal(context, token, total.toString(), getOrderDetail().toJson()).then((res) {
      print(res + "paypal********************");
    }).catchError((error) {
      showLoading(false);
      toast(error.toString());
  ...
  Widget build(BuildContext context) {
    ...
    Widget paymentOptions = Column(
      ...
      children: <Widget>[
        Text(keyString(context, "lbl_payment_method"), style: boldTextStyle(color: context.theme.textTheme.headline6.color)).paddingAll(16).visible(isPayPalEnabled || isPayTmEnabled),
        Row(
            ...
            Container(
              ...
              child: Column(
                ...
                children: <Widget>[
                    ...
                    children: <Widget>[
                      ...
                      Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            color: paymentMethod == PAYTM ? Theme.of(context).primaryColor : null,
                            border: paymentMethod == PAYTM ? null : Border.all(color: Colors.grey, width: 0.5),
                          ),
                          child: Icon(
                            Icons.done,
                            color: Colors.white,
                  ...
                  Text(keyString(context, "lbl_paytm"), style: boldTextStyle(color: context.theme.textTheme.headline6.color)),
                ],
              ),
            )
                .onTap(() {
            ...
            Container(
              ...
              child: Column(
                ...
                children: <Widget>[
                  Row(
                    ...
                    children: <Widget>[
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          shape: BoxShape.circle,
                          color: paymentMethod == PAYPAL ? Theme.of(context).primaryColor : null,
                          border: paymentMethod == PAYPAL ? null : Border.all(color: Colors.grey, width: 0.5),
                        ),
                        child: Icon(
                          Icons.done,
                          color: Colors.white,
                  ...
                  Text(keyString(context, "lbl_paypal"), style: boldTextStyle(color: context.theme.textTheme.headline6.color)).paddingTop(8.0),
                ],
              ),
            )
                .onTap(
    ...
    return Container(
      ...
      child: IntrinsicHeight(
        child: Row(
          ...
          children: <Widget>[
            cachedImage(cartItem.front_cover, height: 30, width: 100, fit: BoxFit.fill).cornerRadiusWithClipRRect(8),
            16.width,
            Column(
              ...
              children: <Widget>[
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: <Widget>[
                    Text(cartItem.name, style: boldTextStyle(color: context.theme.textTheme.headline6.color)),
                    6.height,
                    Text(cartItem.author_name, style: primaryTextStyle(color: context.theme.textTheme.headline6.color)),
                    6.height,
                    Row(
                      children: <Widget>[
                        Text(
                          cartItem.discount != 0
                              ? discountedPrice(tryParse(cartItem.price.toString()), tryParse(cartItem.discount.toString())).toString().toCurrencyFormat()
                        ...
                        6.width,
                        Text(cartItem.discount.toString() + keyString(context, "lbl_off"), style: boldTextStyle(color: Colors.red)).visible(cartItem.discount != 0),
                      ],
                    ),
                  ],
                ),
                  ...
                  children: <Widget>[
                    Row(
                      children: <Widget>[
                        ...
                        2.width,
                        Text(keyString(context, "lbl_move_to_wishlist"), style: boldTextStyle(size: 14, color: colorPrimary)),
                      ],
                    ).onTap(() {
                      removeBookFromCart(context, cartItem, addToWishList: true);
                    }).expand(),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: <Widget>[
