...
class _BookDescriptionScreen2State extends State<BookDescriptionScreen2> with SingleTickerProviderStateMixin {
  ...
  void loadBookFromOffline() async {
    if (mBookDetail != null) {
      ...
      var sampleTask;
      var purchaseTask;
      List<DownloadedBook> list = await dbHelper.queryRowBook(mBookDetail.bookId.toString());
      ...
      if (list != null && list.isNotEmpty) {
        list.forEach((book) {
          ...
          if (book.fileType == "purchased") {
            purchaseBook = book;
            purchaseTask = tasks?.firstWhere((task) => task.taskId == book.taskId);
          }
        });
      }
      if (sampleTask == null) {
  ...
  readBook(context) async {
    if (await Permissions.storageAndAudioGranted()) {
      if (mBookDownloadTask.mDownloadTask.status == DownloadTaskStatus.undefined) {
        var id = await requestDownload(context: context, downloadTask: mBookDownloadTask, isSample: false);
        setState(() {
        ...
        log("jalsa ${mBookDownloadTask.status}");
        readFile(context, mBookDownloadTask.mDownloadTask.filename, mBookDetail.name);
      } else {
        toast('Downloading');
      }
    }
  ...
  sampleClick(context) async {
    if (await Permissions.storageAndAudioGranted()) {
      if (mSampleDownloadTask.status == DownloadTaskStatus.undefined) {
        var id = await requestDownload(context: context, downloadTask: mSampleDownloadTask, isSample: true);
        setState(() {
        ...
        readFile(context, mSampleDownloadTask.mDownloadTask.filename, mBookDetail.name);
      } else {
        toast('Downloading');
      }
    }
  ...
  addRemoveToWishList(isWishList) async {
    bool result = await addRemoveWishList(context, mBookDetail.bookId, isWishList);
    if (result ?? false) {
      setState(() {});
    }
  }
    ...
    isNetworkAvailable().then((bool) {
      if (bool) {
        if (userReviewData != null) {
          var request = {"book_id": mBookDetail.bookId, "user_id": id, "rating_id": userReviewData.ratingId, "rating": rating.toString(), "review": text};
          showLoading(true);
          ...
          var request = {"book_id": mBookDetail.bookId, "user_id": id, "rating": rating.toString(), "review": text, "message": "", "status": true};
          showLoading(true);
              ...
              child: Column(
                ...
                children: <Widget>[
                  Container(
                    ...
                    child: Column(
                      children: <Widget>[
                        Text(keyString(context, "lbl_rateBook"), style: boldTextStyle(size: 24, color: context.theme.textTheme.headline6.color)).paddingAll(10.0),
                        Divider(
                          thickness: 0.5,
                        ),
                        RatingBar.builder(
                        ...
                        Form(
                          ...
                          child: TextFormField(
                            ...
                            decoration: new InputDecoration(
                              ...
                              labelStyle: TextStyle(fontSize: 16, color: context.theme.textTheme.headline6.color),
                          ...
                          children: <Widget>[
                            Expanded(
                              child: MaterialButton(
                                ...
                                child: Text(keyString(context, "aRate_lbl_Cancel"), style: primaryTextStyle(color: context.theme.textTheme.headline6.color)),
                                shape: RoundedRectangleBorder(
                                  borderRadius: new BorderRadius.circular(5.0),
                                  side: BorderSide(color: context.theme.textTheme.headline6.color),
                                ),
                                onPressed: () {
                            ...
                            Expanded(
                              child: MaterialButton(
                                ...
                                child: Text(keyString(context, "lbl_post"), style: primaryTextStyle(color: context.theme.textTheme.headline6.color)),
                                shape: RoundedRectangleBorder(
                                  borderRadius: new BorderRadius.circular(5.0),
                                ),
                                onPressed: () {
  ...
  Widget buildActionForTask() {
    if (mSampleDownloadTask == null) {
      return Text(keyString(context, "lbl_download_sample"), style: primaryTextStyle(color: context.theme.textTheme.headline6.color));
    }
    if (mSampleDownloadTask.status == DownloadTaskStatus.undefined) {
      return Text(keyString(context, "lbl_download_sample"), style: primaryTextStyle(color: context.theme.textTheme.headline6.color));
    } else if (mSampleDownloadTask.status == DownloadTaskStatus.complete) {
      return Text(keyString(context, "lbl_view_sample"), style: primaryTextStyle(color: context.theme.textTheme.headline6.color));
    } else {
      return Text(keyString(context, "lbl_downloading"), style: primaryTextStyle(color: context.theme.textTheme.headline6.color));
    }
  }
      ...
      appBar: appBarWidget(
        ...
        actions: [
          IconButton(
              ...
              onPressed: () {
                Share.share(mBookDetail.name + " by " + mBookDetail.authorName + "\n" + mBaseUrl + "book/detail/" + mBookDetail.bookId.toString());
              }),
          IconButton(
              icon: SvgPicture.asset(mBookDetail.isWishList == 0 ? icon_bookmark : icon_bookmark_fill, color: context.theme.iconTheme.color),
              onPressed: () {
                                    ...
                                    Column(
                                      ...
                                      children: [
                                        ...
                                        Container(
                                          ...
                                          child: Row(
                                            ...
                                            children: <Widget>[
                                              ...
                                              Text(double.parse(mBookDetail.totalRating.toStringAsFixed(1)).toString(), style: primaryTextStyle(color: context.theme.textTheme.headline6.color)).paddingOnly(left: 8),
                                            ],
                                          ),
                                        ),
                                        8.height,
                                        Row(
                                          mainAxisAlignment: MainAxisAlignment.center,
                                          children: <Widget>[
                                            Text(mBookDetail.discountedPrice != 0 ? mBookDetail.discountedPrice.toString().toCurrencyFormat() : mBookDetail.price.toString().toCurrencyFormat(),
                                                    style: boldTextStyle(color: context.theme.textTheme.headline6.color, size: 22))
                                                .visible(mBookDetail.discountedPrice != 0 || mBookDetail.price != 0),
                                            Text(mBookDetail.price.toString().toCurrencyFormat(), style: primaryTextStyle(color: context.theme.textTheme.headline6.color, decoration: TextDecoration.lineThrough))
                                                .paddingOnly(left: 8.0)
                          ...
                          children: <Widget>[
                            ...
                            mBookDetail.is_purchase == 0 && mBookDetail.price != 0
                                ? AppButton(
                                    ...
                                    child: Text('Add to Cart', style: boldTextStyle(color: Colors.white)),
                                    onTap: () {
                                      if (getBoolAsync(IS_LOGGED_IN)) {
                                        if (isExistInCart) {
                            ...
                            mBookDetail.is_purchase == 1 || mBookDetail.price == 0
                                ? AppButton(
                                    color: Theme.of(context).colorScheme.secondary,
                                    child: Text('Read Book', style: boldTextStyle(color: Colors.white)),
                                    onTap: () {
                                      readBook(context);
                                    },
                                  ).expand()
                ...
                child: Column(
                  children: [
                    TabBar(
                      ...
                      tabs: [
                        Tab(child: Text("Overview", style: boldTextStyle(color: context.theme.textTheme.headline6.color))),
                        Tab(child: Text("Information", style: boldTextStyle(color: context.theme.textTheme.headline6.color))),
                        Tab(child: Text("Review", style: boldTextStyle(color: context.theme.textTheme.headline6.color))),
                      ],
                    ),
                    TabBarView(
                      controller: tabController,
